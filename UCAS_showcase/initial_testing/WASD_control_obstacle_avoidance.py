...
class ObstacleAvoidControlWASD:
    ...
    def manual_control_wasd(self):
        ...
        if self.user_input in speedInput:
            if self.user_input != self.previous_speed_input:
                self.tb.motors.set_robot_speed_by_level(int(self.user_input))
    ...
    def adjust_backwards(self):
        ...
        self.tb.motors.change_direction(direction)
        self.tb.motors.set_robot_speed_by_level(int(self.previous_speed_input));
...
if __name__ == '__main__':
    try:
        tb = TurtleBot()
        wasd_control = ObstacleAvoidControlWASD(tb);
        ...
        while(True):
            ...
            if source is not None:
                ...
                for colour in colours:
                    mask, masked_image = tb.camera.detect_colour(image, colour, show_frame=False, frame_name=colour)
