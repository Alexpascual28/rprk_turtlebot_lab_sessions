    ...
    try:
        ...
        while(True):
            if cd.current_hsv_value != previous_hsv_value:
                tb.camera.change_hsv_values(cd.current_colour, cd.current_parameter, cd.current_range, cd.current_hsv_value)
                previous_hsv_value = cd.current_hsv_value

            image = tb.camera.get_current_image()

            if image is not None:
                tb.camera.detect_colour(image, cd.current_colour, frame_name="Colour Mask")
